...
import { Play, Heart, MoreHorizontal, Clock, Trash2, ArrowLeft, Image as ImageIcon } from 'lucide-react';
...
export default function PlaylistDetail() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [playlistData, trackData] = await Promise.all([getPlaylistById(id), getPlaylistTracks(id)]);
  ...
  const handleRemoveTrack = async (e: React.MouseEvent, mediaItemId: string) => {
    ...
    if (!id) return;
    try {
      await removeMediaFromPlaylist(id, mediaItemId);
      setTracks((prev) => prev.filter((t) => t.id !== mediaItemId));
    } catch (error) {
  ...
  const coverUrl = playlist.coverImageUrl ? resolveAssetUrl(playlist.coverImageUrl) : (tracks[0]?.coverImageUrl ? resolveAssetUrl(tracks[0].coverImageUrl) : null);
          ...
          {tracks.map((track, index) => (
            ...
            >
              ...
              <div className="flex justify-center">
                <button
                  onClick={(e) => void handleRemoveTrack(e, track.id)}
